        ...
        <main id="main-scroll" class="chat-main">\r
          ...
          >\r
            <div class="welcome-avatar">\r
              <img style="width: 100%" src="./img/zhai.gif" alt="" />\r
...
\r
          ...
          <div id="chat-list" v-show="showChat || propsText" class="chat-list">\r
            <div\r
              ...
              :class="msg.role"\r
            >\r
              <div class="avatar" :class="msg.role" v-if="msg.role === 'ai'">\r
                <img style="width: 100%" src="./img/zhai.gif" alt="" />\r
              </div>\r
              <div\r
                class="bubble"\r
        ...
        >\r
          <div class="input-wrapper">\r
            <textarea\r
              v-model="userInput"\r
              rows="1"\r
              :placeholder="'输入您的问题...'"\r
              @input="autoResizeTextarea"\r
              @keydown="handleKeydown"\r
              ref="textareaRef"\r
            ></textarea>\r
\r
            <button id="send-btn" class="send-btn" @click="toggleSend">\r
...
if (window.name) {\r
  ...
  conversation_id.value = NAME.conversation_id || "";\r
}\r
\r
...
const mockLoadSDK = () => {\r
  ...
  setTimeout(() => {\r
    ...
    avatarPlatform.writeCmd("action", "A_RH_sit_hello_O");\r
  }, 500);\r
};\r
...
\r
  avatarPlatform\r
    ...
    .on(SDKEvents.frame_stop, () => {\r
      console.log("sdk event: frame_stop, 播报结束");\r
      stopAvatarActions();\r
    })\r
...
\r
const avatarActions = [\r
  "A_RH_emphasize2_O",\r
  "A_RH_sit_introduced_O",\r
  "A_LRH_emphasize2_twice_O",\r
];\r
let actionTimer = null;\r
let currentActionIndex = 0;\r
\r
const startAvatarActions = () => {\r
  stopAvatarActions();\r
  currentActionIndex = 0;\r
  avatarPlatform.writeCmd("action", avatarActions[currentActionIndex]);\r
  currentActionIndex = (currentActionIndex + 1) % avatarActions.length;\r
  actionTimer = setInterval(() => {\r
    if (avatarPlatform) {\r
      avatarPlatform.writeCmd("action", avatarActions[currentActionIndex]);\r
      currentActionIndex = (currentActionIndex + 1) % avatarActions.length;\r
    }\r
...
\r
const stopAvatarActions = () => {\r
  if (actionTimer) {\r
    clearInterval(actionTimer);\r
    actionTimer = null;\r
  }\r
  currentActionIndex = 0;\r
};\r
...
\r
  fullTextBuffer = "";\r
  aiMessageIndex = appendMessage("ai", "", false, true);\r
...
\r
  const data = {\r
    query: query,\r
\r
    // 可能还需要其他字段，如 user_id, conversation_id 等\r
\r
    // 如果是 Dify 格式，通常需要 inputs: {}, response_mode: "streaming" 等\r
\r
    inputs: {},\r
\r
    response_mode: "streaming",\r
\r
    conversation_id: conversation_id.value,\r
\r
    user: "test",\r
  };\r
  const response = await fetch("http://111.33.64.98:28088/v1/chat-messages", {\r
    method: "POST",\r
\r
    headers: {\r
      "Content-Type": "application/json",\r
\r
      Authorization: "Bearer app-fy3hdhJ7M7UtgPI3zLfFVzDe",\r
    },\r
\r
...
\r
  ...
  if (!response.ok) {\r
    throw new Error(\`请求失败：\${response.status}\`);\r
  }\r
\r
  if (!response.body) {\r
    throw new Error("后端未返回流数据");\r
  }\r
...
\r
    // console.log("原始chunk:", chunk); // 调试用\r
\r
    sseBuffer += chunk;\r
\r
    // 解析 SSE 消息\r
\r
    const { messages: sseMessages, incompleteMessage } =\r
      parseSSEMessages(sseBuffer);\r
\r
    for (const message of sseMessages) {\r
      try {\r
        // 通常 SSE 数据格式为 data: {...}\r
\r
        const jsonStr = message.replace(/^data:\\s*/, "").trim();\r
        if (!jsonStr) continue;\r
\r
        const chunkText = JSON.parse(jsonStr);\r
        console.log(123123123, chunkText);\r
        conversation_id.value = chunkText.conversation_id;\r
        // console.log("解析后的SSE消息:", chunkText);\r
\r
        // 根据实际返回结构调整这里的判断逻辑\r
\r
        // 假设结构为 { event: "message", answer: "..." } 或者直接是 { answer: "..." }\r
\r
        if (chunkText.event === "message" || chunkText.answer) {\r
          const content = chunkText.answer || chunkText.content || "";\r
\r
          processChunk(content);\r
        }\r
      } catch (error) {\r
        // 忽略解析错误，通常是 JSON 还没接收完整\r
        // console.error("解析SSE消息失败:", error, message);\r
      }\r
    }\r
\r
    sseBuffer = incompleteMessage;\r
  }\r
\r
  // let i = 0;\r
...
let fullTextBuffer = "";\r
function processChunk(text) {\r
  if (!text) return;\r
\r
  fullTextBuffer += text;\r
  if (aiMessageIndex !== null && messages.value[aiMessageIndex]) {\r
    messages.value[aiMessageIndex].text = fullTextBuffer;\r
  }\r
  scrollToBottom();\r
}\r
function processSSEBuffer(buffer) {\r
  const { messages: sseMessages } = parseSSEMessages(buffer);\r
  for (const message of sseMessages) {\r
    try {\r
      const jsonStr = message.replace(/^data:\\s*/, "").trim();\r
      if (!jsonStr) continue;\r
...
function parseSSEMessages(buffer) {\r
  const messages = [];\r
\r
  let incompleteMessage = "";\r
\r
  // SSE 标准以 \\n\\n 分隔消息\r
\r
  const parts = buffer.split(/\\n\\n/);\r
\r
  // 最后一个部分可能是不完整的，留到下一次处理\r
\r
  if (parts.length > 1) {\r
    const completeParts = parts.slice(0, -1);\r
\r
    incompleteMessage = parts[parts.length - 1];\r
\r
    for (const part of completeParts) {\r
      if (part.trim() !== "") {\r
        messages.push(part.trim());\r
      }\r
    }\r
  } else {\r
    incompleteMessage = buffer;\r
  }\r
\r
  return { messages, incompleteMessage };\r
}\r
\r
// ==================== 生命周期钩子 ====================\r
onMounted(() => {\r
...
\r
  ...
  background: linear-gradient(to right, #0843a8, #4dd5a5);\r
  svg {\r
    width: 40px;\r
    height: 40px;\r
  }\r
...
\r
  ...
  --chat-bg-user: linear-gradient(to right, #0843a8, #4dd5a5);\r
  --chat-text-user: #ffffff;\r
\r
  --chat-bg-ai: #323553; /* 深紫色偏灰 */\r
...
\r
  ...
  --shadow-card: 0 5px 20px rgba(0, 0, 0, 0.3);\r
...
.welcome-avatar::after {\r
  content: "";\r
  position: absolute;\r
  inset: 0;\r
  background-color: rgba(44, 205, 220, 0.62);\r
  mix-blend-mode: overlay; /* 叠加模式 */\r
...
.suggestion-chip:hover {\r
  transform: translateY(-2px);\r
  box-shadow: 0 6px 16px rgba(133, 240, 255, 0.2);\r
  border-color: rgba(133, 240, 255, 0.5);\r
}\r
\r
/* 聊天消息列表 */\r
.chat-list {\r
...
\r
  /* 你要求的边框：色值#2CCCDC，不透明度62% */\r
  border: 2px solid rgba(44, 205, 220, 0.62);\r
\r
  overflow: hidden;\r
  position: relative;\r
}\r
\r
/* 叠加滤镜效果（overlay 混合模式）*/\r
.avatar::after {\r
  content: "";\r
  position: absolute;\r
  inset: 0;\r
...
.input-wrapper:focus-within {\r
  ...
  box-shadow: 0 2px 30px rgb(40, 134, 167, 0.25);\r
  border: 1px solid rgba(133, 240, 255, 0.5);\r
}\r
\r
textarea {\r
  flex: 1;\r
